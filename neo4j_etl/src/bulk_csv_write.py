...
def connect_with_retry(uri, auth, max_retries=10, delay=3):
    """Retry connection until Neo4j is ready"""
...
def load_hospital_graph_from_csv() -> None:
    ...
    driver = connect_with_retry(NEO4J_URI, auth=(NEO4J_USERNAME, NEO4J_PASSWORD))

    LOGGER.info("Setting uniqueness constraints on nodes: nodes must have a unique ID")
